...
df_deacot = pd.read_csv(str(data_dir / 'processed_project_data.csv'), header = 0)
df_deacot = df_deacot.loc[~(df_deacot['collection_symbol'].isna()), :]
...
####################################################
da_list = df_deacot['collection_symbol'].unique()
