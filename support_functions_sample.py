...
def make_sentiment_chart(df, commodity):
    ...
    fig.update_layout(
        newshape=dict(line_color="yellow"),
        title= 'Price Statistics Over Time',
        xaxis_title="",
        yaxis_title="SOL",
    )
    # fig.show(config=lc.tool_config)
...
def make_chart_DA(df, commodity, units):
    ...
    # Add traces
    fig.add_trace(
        go.Scatter(x=df.index, y=df['Volume'], # replace with your own data source
        name="Volume"), secondary_y=False,
    )

    fig.add_trace(
        go.Scatter(x=df.index, y=df['Count'], name="Count"),
        secondary_y= True,
    )
    # Set x-axis title
    ...
    fig.update_layout(
        newshape=dict(line_color="yellow"),
        title= "Volume (SOL) and Count Data",
        xaxis_title="",
        yaxis_title=units,
    )
    # fig.show(config=lc.tool_config)
...
def aggregate_data(df, outlier):
    sample = df.loc[(df['price_change'].abs() <= outlier), :].reset_index(drop=True)
    ...
    count_df = (sample.resample('D', on='datetime').apply(lambda x: len(x['mint'].unique())))
    daily_df = pd.concat([daily_df, med_df, floor_df, volume_df, count_df], axis=1).fillna(method='ffill')
    daily_df.columns = ['Avg Price', 'Median Price', 'Floor Price', 'Volume', 'Count']
    return daily_df
